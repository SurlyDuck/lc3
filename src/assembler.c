...
#define GET_NUM_AND_WARNING(str, res, line) \
	uint8_t c = 0;\
	for(int i = 0; str[i] != '\0'; ++i){ \
		c++;\
	}\
	if(str[0] >= '0' && str[0] <= '9'){\
		WARNING_MESSAGE_LONG("Immediate value without prefix, assuming decimal",line+1,str); \
		STR_TO_INT_NOPREFIX(str, c, &res, 10);\
	} else STR_TO_INT(str, c, &res);
